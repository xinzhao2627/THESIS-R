package com.example.finalthesisapp.Detectors;
...
import com.example.finalthesisapp.ToolsNLP.Recognizer;
import com.example.finalthesisapp.ToolsNLP.Tokenizers.LSTM_tokenizer;
import com.example.finalthesisapp.Types.DetectionResult;
import com.example.finalthesisapp.Types.ModelTypes;
import com.example.finalthesisapp.Types.TextResults;
...
//            options.setUseNNAPI(true);
    ...
    public List<DetectionResult> detect(Bitmap bitmap) {
        ...
        for (TextResults t : textResults) {
            ...
            Log.i(TAG, "detect: nowt: cleaner: " + (System.currentTimeMillis() - nowt));
...
//            Log.i(TAG, "\n");
            if (modelPath.equals(ModelTypes.BILSTM_MODEL)){
                if (l.equals("nsfw") && max_cfs >= 0.95) {
                    Log.i(TAG, "bilstm detect: finaltext: " + text + " conf: " + max_cfs + " (raw): " + t.textContent);
                ...
                if (l.equals("nsfw")) {
                    Log.i(TAG, "lstm only, detect: finaltext: " + text + " conf: " + max_cfs + " (raw): " + t.textContent);
                    detectionResultList.add(new DetectionResult(
                            0,
                            max_cfs,
                            t.left / bitmap.getWidth(),
